...
case $choice in
  [Yy])
    ...
    sudo wget -L https://raw.githubusercontent.com/Nyessss/picomputer/main/system/config.txt -O /boot/config.txt
...
case $choice in
  [Yy])
    ...
    sudo awk '{$0 = $0 " video=HDMI-A-1:1920x480@60D console=serial0 logo.nologo fbcon=rotate:3"} 1' /boot/cmdline.txt | sudo tee /boot/cmdline.txt >/dev/null
